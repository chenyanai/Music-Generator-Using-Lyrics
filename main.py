...
X_train, y_train, x_test, tokenizer, embedding_matrix, vocab_size = \
    Preprocessing.pp_pipeline(data_folder=r'Data', melody_method=melody_type, save_data=start_fresh, seq_len=SEQ_LEN)
...
for Xi_test in x_test:
    song = Model.generate_song(model, Xi_test, words_dict=word_map, reverse_word_dict=reverse_word_map)
    print(song)