    ...
    for file in os.listdir(path):
        ...
        if song_name in train_index['song_name'].values:
            try:
                ...
                pm.lyrics = str(train_index[train_index['song_name'] == song_name]['lyrics'])
                train_midi_dict[file[:-4]] = pm
            except:
                print(file)
        elif song_name in test_index['song_name'].values:
            try:
                pm = pretty_midi.PrettyMIDI(file_path)
                pm.lyrics = str(test_index[test_index['song_name'] == song_name]['lyrics'])
                train_midi_dict[file[:-4]] = pm
            except:
                print(file)
